...
const buildRegPerCategory = (activeCats, numberCols, registerColNames, transToPost, regColIndex, subCatNames) => {
  ...
  activeCats.forEach((cat) => {
    ...
    buildRegDetailPerTrans(transToPost, cat, rowNumber, numberCols, regColIndex, subCatNames, regBodyVals);
    regBodyVals.push(blankLine);
    rowNumber++;
    const catTotalsLine = [];
    for (let i = 0; i < numberCols; i++) {
...
const buildRegDetailPerTrans = (transToPost, cat, rowNumber, numberCols, regColIndex, subCatNames, regBodyVals) => {
