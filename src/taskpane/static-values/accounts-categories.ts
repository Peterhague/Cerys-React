import { FSCategory } from "../interfaces/interfaces";

export const TO_CAT: FSCategory = { short: "Turnover", shortTwo: null, long: "Turnover", sum: false, total: false };
export const COS_CAT: FSCategory = {
  short: "Cost of sales",
  shortTwo: null,
  long: "Cost of sales",
  sum: false,
  total: false,
};
export const OOI_CAT: FSCategory = {
  short: "Other operating income",
  shortTwo: null,
  long: "Other operating income",
  sum: false,
  total: false,
};
export const VAL_ADJ_CAT: FSCategory = {
  short: "Value adjustments on fixed assets",
  shortTwo: "and current asset investments",
  long: "Value adjustments on fixed assets and current asset investments",
  sum: false,
  total: false,
};
export const GP_CAT: FSCategory = {
  short: "Gross profit",
  shortTwo: null,
  long: "Gross profit",
  sum: true,
  total: false,
};
export const GL_CAT: FSCategory = { short: "Gross loss", shortTwo: null, long: "Gross loss", sum: true, total: false };
export const DIST_COSTS_CAT: FSCategory = {
  short: "Distribution costs",
  shortTwo: null,
  long: "Distribution costs",
  sum: false,
  total: false,
};
export const ADMIN_EXES_CAT: FSCategory = {
  short: "Administrative expenses",
  shortTwo: null,
  long: "Administrative expenses",
  sum: false,
  total: false,
};
export const OP_CAT: FSCategory = {
  short: "Operating profit",
  shortTwo: null,
  long: "Operating profit",
  sum: true,
  total: false,
};
export const OL_CAT: FSCategory = {
  short: "Operating loss",
  shortTwo: null,
  long: "Operating loss",
  sum: true,
  total: false,
};
export const PROF_ORD_ACTS: FSCategory = {
  short: "Profit on ordinary activities before interest",
  shortTwo: null,
  long: "Profit on ordinary activities before interest",
  sum: true,
  total: false,
};
export const LOSS_ORD_ACTS: FSCategory = {
  short: "Loss on ordinary activities before interest",
  shortTwo: null,
  long: "Loss on ordinary activities before interest",
  sum: true,
  total: false,
};
export const INT_REC_CAT: FSCategory = {
  short: "Interest receivable and other income",
  shortTwo: null,
  long: "Other interest receivable and similar income",
  sum: false,
  total: false,
};
export const PBIT_CAT: FSCategory = {
  short: "Profit before interest and taxation",
  shortTwo: null,
  long: "Profit before interest and taxation",
  sum: true,
  total: false,
};
export const LBIT_CAT: FSCategory = {
  short: "Loss before interest and taxation",
  shortTwo: null,
  long: "Loss before interest and taxation",
  sum: true,
  total: false,
};
export const INT_PAY_CAT: FSCategory = {
  short: "Interest payable and similar charges",
  shortTwo: null,
  long: "Interest payable and similar charges",
  sum: false,
  total: false,
};
export const PBT_CAT: FSCategory = {
  short: "Profit before taxation",
  shortTwo: null,
  long: "Profit before taxation",
  sum: true,
  total: false,
};
export const LBT_CAT: FSCategory = {
  short: "Loss before taxation",
  shortTwo: null,
  long: "Loss before taxation",
  sum: true,
  total: false,
};
export const TAX_PROF: FSCategory = {
  short: "Taxation on profit",
  shortTwo: null,
  long: "Taxation on profit",
  sum: false,
  total: false,
};
export const TAX_LOSS: FSCategory = {
  short: "Taxation on loss",
  shortTwo: null,
  long: "Taxation on loss",
  sum: false,
  total: false,
};
export const PROFIT: FSCategory = {
  short: "Profit for the financial year",
  shortTwo: null,
  long: "Profit for the financial year",
  sum: true,
  total: true,
};
export const LOSS: FSCategory = {
  short: "Loss for the financial year",
  shortTwo: null,
  long: "Loss for the financial year",
  sum: true,
  total: true,
};
